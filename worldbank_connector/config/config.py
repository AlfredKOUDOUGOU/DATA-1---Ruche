...
config.py
=========
Chargement et validation de la configuration du connecteur Banque mondiale.
...
class Config:
    ...
    wb_api_base_url: str = field(
        default_factory=lambda: os.getenv("WB_API_BASE_URL", "https://api.worldbank.org/v2")
    )
    wb_api_timeout: int = field(default_factory=lambda: _get_int("WB_API_TIMEOUT", 60))
    target_country_code: str = field(
        default_factory=lambda: os.getenv("TARGET_COUNTRY_CODE", "BFA")
    )

    # ID de source WB optionnel (ex: 2 = World Development Indicators).
    # Si None, le connecteur scanne l'integralite du catalogue d'indicateurs.
    wb_source_id: Optional[str] = field(default_factory=lambda: _get_optional_str("WB_SOURCE_ID"))

    # --- Limitation des requetes / retry ---
    wb_max_requests_per_minute: int = field(
        default_factory=lambda: _get_int("WB_MAX_REQUESTS_PER_MINUTE", 120)
    )
    wb_max_retries: int = field(default_factory=lambda: _get_int("WB_MAX_RETRIES", 3))
    wb_retry_backoff: int = field(default_factory=lambda: _get_int("WB_RETRY_BACKOFF", 5))

    # --- Chemins locaux ---
    data_dir: Path = field(
        default_factory=lambda: (PROJECT_ROOT / os.getenv("DATA_DIR", "./data")).resolve()
    )
    logs_dir: Path = field(
        default_factory=lambda: (PROJECT_ROOT / os.getenv("LOGS_DIR", "./logs")).resolve()
    )
    ...
    max_indicators_to_scan: Optional[int] = field(
        default_factory=lambda: _get_optional_int("MAX_INDICATORS_TO_SCAN")
    )
    skip_coverage_check: bool = field(
        default_factory=lambda: _get_bool("SKIP_COVERAGE_CHECK", False)
    )
    ...
    def validate(self) -> None:
        ...
        if not self.wb_api_base_url:
            missing.append("WB_API_BASE_URL")
