...
main.py
=======
Point d'entree du connecteur OCDE - BF Pulse.

Orchestre l'ensemble du pipeline :
    1. Chargement de la configuration (.env).
    2. Connexion a l'API OCDE et a PostgreSQL.
    3. Decouverte des dataflows couvrant le Burkina Faso.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Connecteur OCDE - BF Pulse : collecte des indicateurs Burkina Faso."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Limite le nombre de dataflows traites (utile pour tester rapidement)",
    )
    parser.add_argument(
        "--skip-db",
        action="store_true",
...
def print_banner() -> None:
    print("=" * 70)
    print(" BF Pulse - Connecteur OCDE ".center(70, "="))
    print("=" * 70)
...
def print_datasets_summary(datasets: List[DatasetMeta]) -> None:
    ...
    if not datasets:
        print("  (aucun dataset trouve)")
        return
    for ds in datasets:
        print(f"  - {ds.name} [{ds.full_id}] ({len(ds.indicators)} indicateur(s))")
    print()
...
def print_final_report(report: RunReport, elapsed_seconds: float) -> None:
    ...
    print("=" * 70)
    print(f"Nombre de datasets recuperes      : {report.datasets_processed}/{report.datasets_found}")
    print(f"Nombre de datasets en echec        : {report.datasets_failed}")
    print(f"Nombre d'indicateurs               : {report.indicators_count}")
...
def run() -> int:
    ...
    print_banner()
    logger.info("Demarrage du connecteur OCDE - BF Pulse.")
    logger.info(
        "Configuration chargee : pays cible=%s, quota API=%d req/h, base=%s",
        config.target_country_code,
        config.oecd_max_requests_per_hour,
        config.oecd_api_base_url,
    )
    ...
    with OECDClient(config) as client:
        ...
        try:
            ...
            logger.error("Echec de la decouverte des dataflows : %s", exc)
        ...
        for dataset_meta in datasets:
            logger.info("Traitement du dataset : %s", dataset_meta.full_id)
            try:
                ...
                df = data_parser.parse_many(
                    file_paths, dataset_meta, config.target_country_code
                )
                ...
                logger.exception("Echec du traitement du dataset %s", dataset_meta.full_id)
