...
class DatabaseManager:
    ...
    def upsert_dataset(self, session: Session, source: Source, dataset_meta: DatasetMeta) -> Dataset:
        dataset = (
            ...
            .filter_by(dataset_code=dataset_meta.dataflow_id, source_id=source.id)
            .one_or_none()
        )
        if dataset is None:
            dataset = Dataset(
                source_id=source.id,
                dataset_code=dataset_meta.dataflow_id,
            ...
            session.add(dataset)
            session.flush()
            self.logger.info("Dataset cree : %s", dataset_meta.dataflow_id)
        else:
            dataset.dataset_name = dataset_meta.name
            dataset.description = dataset_meta.description
            dataset.last_update = datetime.utcnow()
            self.logger.debug("Dataset mis a jour : %s", dataset_meta.dataflow_id)
        return dataset
    ...
    def load_dataframe(
        ...
        df: pd.DataFrame,
        source_name: str = "OCDE",
        source_url: str = "https://sdmx.oecd.org",
        source_description: str = "Organisation de Cooperation et de Developpement Economiques",
    ) -> int:
        ...
        """
        if df.empty:
            self.logger.info("Aucune observation a charger pour %s.", dataset_meta.dataflow_id)
            return 0
