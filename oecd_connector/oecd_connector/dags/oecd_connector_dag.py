"""
oecd_connector_dag.py
======================
DAG Apache Airflow permettant d'executer automatiquement le connecteur
OCDE (BF Pulse) chaque semaine.

Installation :
    1. Copier ce fichier dans le dossier `dags/` de votre instance Airflow.
    2. Copier (ou monter) le dossier `oecd_connector/` complet sur la
       machine/le worker Airflow, par exemple sous `/opt/bf_pulse/oecd_connector`.
...
Le DAG s'execute tous les lundis a 03h00 (UTC) et :
    - installe/verifie les dependances (optionnel, via un venv dedie) ;
...
CONNECTOR_DIR = "/opt/bf_pulse/oecd_connector"
...
with DAG(
    dag_id="bf_pulse_oecd_connector",
    description="Collecte hebdomadaire des indicateurs OCDE pour le Burkina Faso (BF Pulse)",
    default_args=default_args,
    schedule_interval="0 3 * * 1",  # Chaque lundi a 03h00 UTC
    start_date=datetime(2026, 1, 1),
    catchup=False,
    max_active_runs=1,
    tags=["bf_pulse", "oecd", "data_engineering"],
) as dag:

    run_oecd_connector = BashOperator(
        task_id="run_oecd_connector",
        bash_command=(
            f"cd {CONNECTOR_DIR} && "
